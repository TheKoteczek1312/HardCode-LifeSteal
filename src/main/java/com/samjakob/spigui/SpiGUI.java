        ...
        switch (type) {
            case PREV_BUTTON:
                if (inventory.getCurrentPage() > 0) return new SGButton(new ItemBuilder(Material.ARROW)
                        .name(Config.getMessage("previousPage"))
                        .lore(Config.getMessage("previousPageLore").replace("${page}", String.valueOf(inventory.getCurrentPage())))
//                        .name("&a&l\u2190 Previous Page")
//                        .lore(
//                                "&aClick to move back to page " + inventory.getCurrentPage() + ".")
                        .build()
                ).withListener(event -> {
            ...
            case CURRENT_BUTTON:
                return new SGButton(new ItemBuilder(Material.NAME_TAG)
                        .name(Config.getMessage("currentPage").replace("${currentPage}", String.valueOf(inventory.getCurrentPage() + 1)).replace("${maxPage}", String.valueOf(inventory.getMaxPage())))
                        .lore(Config.getMessage("currentPageLore").replace("${page}", String.valueOf(inventory.getCurrentPage() + 1)))
//                        .name("&7&lPage " + (inventory.getCurrentPage() + 1) + " of " + inventory.getMaxPage())
//                        .lore(
//                                "&7You are currently viewing page " + (inventory.getCurrentPage() + 1) + ".")
                        .build()
            ...
            case NEXT_BUTTON:
                if (inventory.getCurrentPage() < inventory.getMaxPage() - 1)
                    return new SGButton(new ItemBuilder(Material.ARROW)
                            .name(Config.getMessage("nextPage"))
                            .lore(Config.getMessage("nextPageLore").replace("${page}", String.valueOf(inventory.getCurrentPage() + 2)))
//                            .name("&a&lNext Page \u2192")
//                            .lore(
//                                    "&aClick to move forward to page " + (inventory.getCurrentPage() + 2) + ".")
                            .build()
                    ).withListener(event -> {
                        event.setCancelled(true);
     ...
     */
    public SpiGUI(final JavaPlugin plugin) {
        this.plugin = plugin;
     ...
     */
    public void setEnableAutomaticPagination(final boolean enableAutomaticPagination) {
     ...
     * @see SpiGUI#defaultPaginationButtonBuilder
     */
    public void setDefaultPaginationButtonBuilder(final SGPaginationButtonBuilder defaultPaginationButtonBuilder) {
     ...
     */
    public List<SGOpenMenu> findOpenWithTag(final String tag) {
        ...
        for (final Player player : this.plugin.getServer().getOnlinePlayers()) {
            ...
            if (player.getOpenInventory().getTopInventory() != null) {
                ...
                if (topInventory.getHolder() != null && topInventory.getHolder() instanceof SGMenu) {
                    ...
                    final SGMenu inventory = (SGMenu) topInventory.getHolder();
